"""
Unibots 2026 — YELLOW Zone
===========================
Assigned zone: YELLOW (top wall, AprilTags 2 & 3).
...
Usage:
  python3 run_yellow.py [--port-a /dev/ttyUSB0] [--port-b /dev/ttyUSB1]
...
# ============================================================
TARGET_ZONE     = "yellow"
DEPOSIT_HEADING = 0          # face south (back into top wall at y=0)

# ============================================================
#  GAME TUNING
# ============================================================
GAME_DURATION    = 180
RETURN_TIME      = 30        # head home with 30 s left — lift one-direction only
ARRIVAL_DIST_MM  = 150       # mm — "arrived at zone centre"
BOTTOM_THRESH    = 210       # ball y-pixel where we commit to intake
INTAKE_TIME      = 1.5       # s — drive forward after committing
SPIN_SPEED       = 120       # PWM while searching
DRIVE_SPEED      = 200       # PWM while chasing
ROTATE_KP        = 0.8       # gain: rotate to centre ball
CENTER_DEAD      = 25        # pixel deadband for horizontal centering
BACK_DURATION    = 1.5       # s — reverse into net
BACK_SPEED       = 80        # PWM for wall nudge
LIFT_UP_DURATION = 2.5       # s — raise lift over net
...
BALL_CAM_W  = 320
BALL_CAM_H  = 240
CAM_HEIGHT  = 150     # mm above floor
CAM_TILT    = 15      # degrees downward
BALL_H_FOV  = 90      # ball camera horizontal FOV
BALL_V_FOV  = 50.625  # ball camera vertical FOV

BAUD = 115200


# ============================================================
#  MOTOR CONTROL  (wheels_control.cpp protocol)
# ============================================================

def send_pwm(ser, m1, m2):
    """Send M1:val,M2:val — matches wheels_control.cpp."""
...
def mecanum(serA, serB, fwd, strafe, rotate):
    """Mecanum kinematics — left side is robot front (matches main.py)."""
...
def set_lift(serA, speed):
    """L:speed → Arduino analogWrite(5, speed). One-direction only — no negatives."""
...
# ============================================================
#  BALL DETECTION  (orange, from working main.py)
# ============================================================

def detect_ball_orange(frame, prev_circle=None):
    """Detect orange ball using HSV mask + Hough circles."""
...
def drive_toward(serA, serB, distance_mm, rotation_deg, speed=DRIVE_SPEED):
    """Drive toward a target given (distance, rotation) from direction_to_zone."""
...
def turn_toward(serA, serB, robot_h, target_h, speed=120, tol=5):
    """Rotate toward target heading. Returns True when within tolerance."""
...
def run(port_a, port_b, tag_cam, ball_cam):
    ...
    # Tag camera (high res for AprilTags)
        ...
        while True:
            ...
            if game_state == "seek_ball":
                ...
                if robot_x is not None and robot_h is not None:
                    ...
                    mecanum(serA, serB, 0, 0, SPIN_SPEED)  # spin to find tags
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port-a", default="/dev/ttyUSB0",
                        help="Arduino A serial port (lift/intake/BR/BL)")
    parser.add_argument("--port-b", default="/dev/ttyUSB1",
                        help="Arduino B serial port (FL/FR)")
    parser.add_argument("--tag-cam",  type=int, default=0,
                        help="Camera index for AprilTag detection")
    parser.add_argument("--ball-cam", type=int, default=1,
                        help="Camera index for ball detection")
